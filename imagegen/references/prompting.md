...
These prompting principles apply to all work done through the skill's `scripts/image_gen.py` CLI.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in `references/cli.md` and `references/image-api.md`.
...
- [CLI execution controls](#cli-execution-controls)
...
- For small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes, use `medium` or `high` quality.
...
## Transparent images
- Default path for transparent-image requests: generate on a flat chroma-key background, then remove the key color locally. If the subject is clearly too complex for chroma-key removal, explain the tradeoff and ask before switching to `gpt-image-1.5` true transparency.
...
- After generation, remove the background locally with `python "<skill-dir>/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
- Use `gpt-image-1.5 --background transparent --output-format png` only after the user explicitly confirms, or when the user already explicitly requested `gpt-image-1.5` or true/native transparency. Ask first for true/native transparency requests, failed chroma-key validation, or complex transparent subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows.

## CLI execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are CLI execution controls passed to the API; they are not prompt scaffolding.
- See `references/cli.md` and `references/image-api.md` for those controls.
- `gpt-image-2` is the CLI default. It supports `quality=low|medium|high|auto`; use `low` for fast drafts and thumbnails, and move to `medium`, `high`, or `auto` for final assets.
- `gpt-image-2` always uses high fidelity for image inputs, so do not set `input_fidelity` with that model.
- If a transparent request needs true model-native transparency, ask before using `gpt-image-1.5` unless the user already explicitly chose it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
