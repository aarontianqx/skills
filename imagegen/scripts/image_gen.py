...
Works against the official OpenAI API or any OpenAI-compatible relay/proxy
endpoint. Credentials and endpoint resolve in this order:
...
def _resolve_base_url(args: argparse.Namespace) -> Optional[str]:
    return getattr(args, "base_url", None) or os.getenv("IMAGE_GEN_BASE_URL")


def _ensure_api_key(args: argparse.Namespace) -> None:
    if _resolve_api_key(args):
        print("API key is set.", file=sys.stderr)
        return
    if args.dry_run:
        _warn("No API key set (OPENAI_API_KEY / IMAGE_GEN_API_KEY); dry-run only.")
        return
    _die(
        "No API key found. Set OPENAI_API_KEY (or IMAGE_GEN_API_KEY), "
        "or pass --api-key."
    )
...
def _create_client(args: argparse.Namespace):
    try:
        from openai import OpenAI
    except ImportError:
        _die(f"openai SDK not installed in the active environment. {_dependency_hint('openai')}")
    return OpenAI(**_client_kwargs(args))


def _create_async_client(args: argparse.Namespace):
    try:
    ...
    return AsyncOpenAI(**_client_kwargs(args))
...
def _add_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--api-key",
        help="API key. Overrides IMAGE_GEN_API_KEY / OPENAI_API_KEY env vars.",
    )
    parser.add_argument(
        "--base-url",
        help=(
            "Base URL of an OpenAI-compatible API (for example a relay/proxy "
            "endpoint). Overrides IMAGE_GEN_BASE_URL / OPENAI_BASE_URL env vars."
        ),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="CLI for image generation or editing via GPT Image models"
    )
