...
These parameters describe the Image API surface exposed by the bundled `scripts/image_gen.py` CLI. The CLI works against the official OpenAI API or any OpenAI-compatible relay/proxy endpoint (see `references/cli.md` for endpoint/key configuration).

## Scope
- This CLI is intended for GPT Image models (`gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, and `gpt-image-1-mini`).
- Relay/proxy endpoints may not offer every model or parameter; surface API errors as-is instead of silently substituting models.
...
| `gpt-image-1.5` | `low`, `medium`, `high`, `auto` | `low`, `high` | `1024x1024`, `1024x1536`, `1536x1024`, `auto` | True transparent-background output and backward-compatible workflows |
...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. The skill's default transparent-image path is generation on a flat chroma-key background, followed by local alpha extraction with `python "<skill-dir>/scripts/remove_chroma_key.py"`.

Use `gpt-image-1.5` with `background=transparent` and a transparent-capable output format such as `png` or `webp` only after the user explicitly confirms that path, unless they already requested `gpt-image-1.5` or true/native transparency. If the user asks for true/native transparency, the subject is too complex for clean chroma-key removal, or local background removal fails validation, explain the tradeoff and ask before switching.
...
## Important boundary
- `quality`, `input_fidelity`, explicit masks, `background`, `output_format`, and related parameters are CLI execution controls passed to the API; they are not prompt scaffolding.
- Not every relay/proxy endpoint supports every parameter; if the endpoint rejects one, report the error plainly.
